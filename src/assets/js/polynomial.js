...
export function renderModelPredictions(
  canvas,
  order,
  model,
  xPowerMeans,
...
export function world2canvas(canvas, x, y) {
...
export function drawXYData(canvas, xyData) {
  drawAxes(canvas);
  const ctx = canvas.getContext("2d");
...
export function normalizeVector(vector, vectorMean, vectorStddev) {
...
export function toNormalizedTensors(xyData, order) {
...
//   yMean: Arithmetic mean of y.
//   yStddev: Standard deviation of y.
export async function fitModel(xyData, epochs, learningRate, order) {
  const batchSize = xyData.length;
...
export function generateXYData(canvas, coeffs) {
  const data = [];
  for (
